...
## Current Milestone Status (2026-07-16)

Milestones are cross-phase approval gates (see the numbering note above); they run **alongside** — and do not replace — the Phase/Sprint structure below.

- **Milestone 1 — Arena Loop Validation: COMPLETE (playtest-approved).** The core Arena gameplay loop is proven fun-to-repeat: the four-enemy greybox roster (**Walker / Charger / Surrounder / Empowerer**) is tuned and playtest-approved, hit/kill/player-damage feedback landed (Blocks 2A–2C — PC-017…PC-020), and the canonical gameplay direction is frozen (**DEC-037**), with player abilities never self-damaging by default (**DEC-038**). This completes the gameplay-validation thread the Phase 2 clusters below were exercising. Detail: [CHANGELOG.md](CHANGELOG.md) (2026-07-13 → 2026-07-15).
- **Milestone 2 — Visual Foundation: IN PRE-PRODUCTION.** The frozen **[Art Bible v1.0](Documentation/Vision/ART_BIBLE.md)** defines the global visual identity (readability laws, saturation budget, semantic-colour lock, the Hollowed faction, the player locator, Arena 01 "The Shattered Coast", performance targets). The approved **[Visual Benchmark Plan](Documentation/Vision/POINT_CLEAR_VISUAL_BENCHMARK_PLAN.md)** is the operational blueprint for the first visual benchmark. **Supporting research (non-canonical):** [Asset Discovery](Documentation/Research/Assets/POINT_CLEAR_ASSET_DISCOVERY.md) (sourcing doctrine), the live-verified [Live Asset Shortlist](Documentation/Research/Assets/POINT_CLEAR_LIVE_ASSET_SHORTLIST.md) (current purchasing evidence), and [Asset Validation](Documentation/Research/Assets/POINT_CLEAR_ASSET_VALIDATION.md) (superseded for purchasing; retained as methodology record). **No visual production (Phase 0) has begun; benchmark execution is gated by provisional-stack approval, live verification, purchase authorization, and an explicit GD go.**

*Relationship to the phases below: Milestone 1 was proven through the Phase 2 vertical-slice work; Milestone 2 pre-production runs in parallel and does not alter the Phase 0/1/2 structure. The per-sprint statuses below remain the granular record; where a sprint predates this summary, this section is the current truth.*
...
- Relationship to Sprint 2.10: this gate tests whether Build Identity exists *at all* — the earliest, cheapest checkpoint, before Mission Risk or Loot exist. **[Reframed 2026-07-12 — DEC-030]** Sprint 2.10 is now the **Behavioral Divergence Check**: it asks whether the build changes *how players engage the world's questions* (behavioral divergence during progression, seduced by world pressure per DEC-023/DEC-024) — **not** whether *terminal* builds avoid converging on one dominant combination. That terminal/structural convergence question is a separate, much-later experiment (requires the finite budget — DEC-021 — and a larger build space). Passing this Cluster A gate remains a prerequisite for a meaningful Sprint 2.10.
...
- Why before 2.10: equipment adds more build tools for players to *reach for differently* under world pressure, enriching what the Behavioral Divergence Check (Sprint 2.10, reframed per DEC-030) can observe. *(Note: Sprint 2.11 now runs between 2.9 and 2.10 — it supplies the second world-question 2.10 requires.)*
...
**Sprint 2.10 — Behavioral Divergence Check** *(reframed 2026-07-12 — DEC-030; was "Build Validation & Convergence Check")*
- **Execution order: runs AFTER Sprint 2.11, not before it.** With only one enemy the check is un-runnable (nothing to diverge over); 2.11 supplies the second world-question the experiment requires.
- Question: Under differential world pressure, does the build **change how players play** (within-subject), and do players settle into **recognizably different, durable approaches** (between-subject)? I.e. is Point Clear *seducing* divergent playstyles (DEC-023/DEC-024), not just letting everyone fill the same checklist? It measures **how players engage the world's questions while progressing — not their final optimized loadouts.**
- Dependencies: Sprint 2.11 (≥2 enemies that genuinely reward different answers — the measuring instrument) + the in-moment friction that already exists (cooldowns/attention). **Does NOT require the finite build budget (DEC-021)** — that gates the separate, much-later *terminal* experiment (see Deferred sibling, below).
- Instrument (DEC-030 — **current and revisable**, not immutable doctrine): **Probe A (within-subject, primary)** — same player, two contrasting builds, does their engagement of the same enemies change? (isolates the build's effect from personality). **Probe B (between-subject, corroboration)** — several players, same start, observe first tool reached for per enemy, hold-vs-reach, first investments, one-sentence self-described playstyle. One page per player; watch behavior, not unlocked content.
- Decision (pre-registered): **PASS** (build-sensitive engagement + durable cross-situational spread) → proceed to Sprint 2.12. **FAIL** (behavior flat across builds, or everyone converges on one approach as pure stimulus-response) → do not add mechanics; expand the interaction space among existing layers, then re-run. **INCONCLUSIVE** (preconditions unmet: fewer than 2 real questions, a grossly dominant approach, or sample too small/short) → not yet runnable; fix preconditions and re-run. Trust a PASS more than a FAIL — a FAIL is provisional (identity may simply not have formed yet); the sprint is repeatable.
- Out of scope: terminal loadout composition, optimized-DPS builds, unlocked-content inventories; any new mechanic, layer, or content.
...
- **Update (2026-07-16):** the four-enemy greybox roster (Walker / Charger / Surrounder / Empowerer) has since been **tuned and playtest-approved** as part of **Milestone 1 — Arena Loop Validation (COMPLETE)** — see the Current Milestone Status section above and [CHANGELOG.md](CHANGELOG.md) (2026-07-15). The Behavioral Divergence Check (Sprint 2.10) framing is unchanged.
...
- Dependencies: Cluster A's build content (the new enemy must specifically challenge something that exists). *(No longer depends on "Sprint 2.10 passed" — the order is reversed; see below.)*
- **Why before 2.10:** it supplies the second world-question without which the Behavioral Divergence Check (Sprint 2.10) cannot run (DEC-030).
